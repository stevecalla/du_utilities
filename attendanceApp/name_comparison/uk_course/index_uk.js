...
import { participantRoster, list_today } from './data.js';
...
let present = maxSimilarityScores.filter(({maxSimilarity}) => maxSimilarity > 0.60);
let absent = maxSimilarityScores.filter(({maxSimilarity}) => maxSimilarity <= 0.60);
...
let inputPlug = maxSimilarityScores.map((score, i) => {
  let status = score.maxSimilarity <= 0.60 ? "Absent" : "Present";
  return status;
  return {
    // index: i,
...
const zoomReport = maxSimilarityScores.
  filter(score => score.maxSimilarity > 0.60).
  map((result, index) => {
    return {
      index: (index + 1),
      name: result.name,
